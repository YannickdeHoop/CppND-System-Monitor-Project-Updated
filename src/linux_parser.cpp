...
long LinuxParser::ActiveJiffies() {
    std::string cpu_name, user, nice, system, idle, iowait, irq, softirq, steal, guest, line;
...
long LinuxParser::IdleJiffies() {
    std::string user, nice, system, idle, iowait, irq, softirq, steal, guest, line;
    std::ifstream stream(kProcDirectory + kStatFilename);
    if (stream.is_open()) {
      std::getline(stream, line);
      std::istringstream linestream(line);
...
std::map<std::string, std::string> LinuxParser::GetUIDUserMap() {
    ...
    std::string user_name, password, uid, group_id, user_id_info, home_dir, cmd, line;
    std::ifstream filestream(kPasswordPath);
    if (filestream.is_open()) {
      while (std::getline(filestream, line)) {
        std::replace(line.begin(), line.end(), ':', ' ');
        std::istringstream linestream(line);
        while (linestream >> user_name >> password >> uid >> group_id >> user_id_info >> home_dir >> cmd) {
...
//2) active_jiffies = (user + nice + system + irq + softirq + steal + guest + guest_nice); in /proc/stat file
//3) idle_jiffies = iowait; in /proc/stat file

//for a Process:

//1) system_jiffies = active_jiffies + idle_jiffies;
//2) active_jiffies = (utime + stime + cutime + cstime); in /proc/[PID]/stat file
//3) idle_jiffies = iowait; in /proc/stat file
